...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		newDeck.describe();	
		newDeck.drawCard();newDeck.drawCard();
		...
		while (players > 4 || players < 2) {
			System.out.println("Please choose between 2 and 4 players.\nNumber of Players: ");
			players = sc.nextInt();
		}
		
		System.out.println(gameBoard(players));
	...
	public static Map<String, List<Card>> gameBoard(int players) {
		...
		if (players == 2) {
			for (int i = 0; i < 52; i++) {
				if (deck.getCards().size()%2 == 0) {
					board.put("Player 1", deck.drawCard());
				} else {
					board.put("Player 2", deck.drawCard());
				}
			}
		}
		...
		if (players == 3) {
			for (int i = 0; i < 52; i++) {
				if (deck.getCards().size()%2 == 0) {
					board.put("Player 1", deck.drawCard());
				} else if (deck.getCards().size()%3 == 0) {
					board.put("Player 2", deck.drawCard());
				} else {
					board.put("Player 3", deck.drawCard());
				}
			}
		}
		...
		if (players == 4) {
			for (int i = 0; i < 52; i++) {
				if (deck.getCards().size()%2 == 0) {
					...
					board.put("Player 2", deck.drawCard());
				} else if (deck.getCards().size()%4 == 0) {
					board.put("Player 3", deck.drawCard());
				} else {
					board.put("Player 4", deck.drawCard());
				}
			}
		} return board;
	}
